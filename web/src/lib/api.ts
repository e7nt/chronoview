import type { Timeline, TimelineWithRole, TimelineVersion, ShareLink, Collaborator } from "./types";

const runtimeConfig = (window as Record<string, unknown>).__APP_CONFIG__ as { VITE_API_URL?: string } | undefined;
const API_BASE = runtimeConfig?.VITE_API_URL
	? `${runtimeConfig.VITE_API_URL}/api`
	: "/api";
...
export const api = {
	...
	deleteTimeline: (id: string) =>
		request<void>(`/timelines/${id}`, { method: "DELETE" }),

	// Sharing
	createShareLink: (timelineId: string, data: { is_public?: boolean; passcode?: string }) =>
		request<ShareLink>(`/timelines/${timelineId}/share`, { method: "POST", body: JSON.stringify(data) }),
	...
	inviteCollaborator: (timelineId: string, data: { email: string; role: string }) =>
		request<Collaborator>(`/timelines/${timelineId}/collaborators`, { method: "POST", body: JSON.stringify(data) }),
	updateCollaborator: (timelineId: string, collabId: string, data: { role: string }) =>
		request<Collaborator>(`/timelines/${timelineId}/collaborators/${collabId}`, { method: "PUT", body: JSON.stringify(data) }),
	...
	createVersion: (timelineId: string, data: { content: string; label?: string }) =>
		request<TimelineVersion>(`/timelines/${timelineId}/versions`, { method: "POST", body: JSON.stringify(data) }),
	getVersion: (timelineId: string, versionId: string) =>
		request<TimelineVersion & { content: string }>(`/timelines/${timelineId}/versions/${versionId}`),
};
