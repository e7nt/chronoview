	...
	return (
		<>
			...
			>
				{/* Header */}
				<div
					className={`flex items-center gap-2 px-4 py-3 border-b ${dark ? "border-stone-700" : "border-stone-100"}`}
				>
					<Inbox size={14} className={dark ? "text-stone-400" : "text-stone-500"} />
					<span
						className={`text-[13px] font-semibold ${dark ? "text-stone-200" : "text-stone-700"}`}
					>
						Unplanned
					</span>
					<span
						className={`text-[10px] px-1.5 py-0.5 rounded-full font-medium ${dark ? "bg-stone-800 text-stone-400" : "bg-stone-100 text-stone-500"}`}
					>
				...
				<div className="flex-1 overflow-y-auto p-3 space-y-4">
					...
					{Array.from(tasksBySection.entries()).map(([sectionName, tasks]) => (
						<div key={sectionName}>
							<p
								className={`text-[10px] font-semibold uppercase tracking-widest mb-2 px-1 ${dark ? "text-stone-500" : "text-stone-400"}`}
							>
								{sectionName}
							</p>
							<div className="space-y-1">
								{tasks.map((task) => (
									...
									>
										...
										<span
											className={`text-[12px] truncate ${dark ? "text-stone-300" : "text-stone-700"}`}
										>
											{task.name}
										</span>
									</div>
				...
				{/* Footer hint */}
				<div
					className={`px-4 py-2.5 border-t text-center ${dark ? "border-stone-700" : "border-stone-100"}`}
				>
