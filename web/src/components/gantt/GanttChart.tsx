...
import type { ChartAnnouncement, ChartTask, ChartTimeline } from "@/lib/build-chart-timeline";
import {
	addDays,
	daysBetween,
	formatDate,
...
export function GanttChart({
	...
	const [selectedAnnouncement, setSelectedAnnouncement] = useState<
		(ChartAnnouncement & { x: number }) | null
	>(null);
		...
		return {
			...
			sections: timeline.sections.map((s) => ({
				...
				tasks: s.tasks.map((t) => {
					...
					return edit
						? { ...t, planned_start: edit.planned_start, planned_end: edit.planned_end }
						: t;
				}),
			})),
		};
	...
	const handleBarDragStart = useCallback(
		(taskId: string, sectionId: string, e: React.PointerEvent) => {
			e.preventDefault();
			e.stopPropagation();
			dragDidMove.current = false;
			const startPointerX = e.clientX;

			const task = timeline.sections
			...
			const onPointerMove = (ev: PointerEvent) => {
				...
				setPendingEdits((prev) =>
					new Map(prev).set(taskId, { planned_start: newStart, planned_end: newEnd }),
				);
			};
			const onPointerUp = () => {
	...
	const handleResizeRight = useCallback(
		(taskId: string, sectionId: string, e: React.PointerEvent) => {
			...
			const onPointerMove = (ev: PointerEvent) => {
				...
				setPendingEdits((prev) =>
					new Map(prev).set(taskId, {
						planned_start: toISODate(startDate),
	...
	const handleTaskUpdate = useCallback(
		(taskId: string, changes: Partial<ChartTask>) => {
	...
	const handleTouchResizeStart = useCallback(
		(e: React.TouchEvent) => {
			...
			const onTouchMove = (ev: TouchEvent) => {
				ev.preventDefault();
				setLabelWidth(
					Math.min(maxLabelW, Math.max(minLabelW, startWidth + ev.touches[0]!.clientX - startX)),
				);
			};
			const onTouchEnd = () => {
				setIsResizing(false);
	...
	const selectTask = useCallback((task: ChartTask | null) => {
		...
		<div className={`flex flex-col h-full ${darkMode ? "dark" : ""}`}>
			...
			<div className="border-b border-border-subtle dark:border-stone-700 bg-white/80 dark:bg-stone-900/90 backdrop-blur-sm overflow-visible relative z-30">
				...
				<div className="flex items-center gap-2 sm:gap-3 px-3 sm:px-4 py-2">
					<h1 className="text-base sm:text-lg font-semibold text-stone-800 dark:text-stone-100 mr-auto truncate">
						{timeline.title}
					</h1>
					...
					<div style={{ minWidth: labelWidth + chartWidth }}>
						...
						<div className="sticky top-0 z-20 flex bg-white dark:bg-stone-900 border-b border-border-subtle dark:border-stone-700">
							...
							>
								...
								>
									{collapsedSections.size === editedTimeline.sections.length
										? "Expand all"
										: "Collapse all"}
								</button>
							</div>
							...
							{selectedAnnouncement && (
								<div
									...
									style={{
										left: Math.max(
											8,
											Math.min(windowWidth - 288, labelWidth + selectedAnnouncement.x - 140),
										),
						...
						<div className="flex relative">
							...
							>
								...
								{rows.map((row, i) => {
									const isCollapsed =
										row.type === "section" && collapsedSections.has(row.sectionId);
									const taskCount =
										row.type === "section"
											? (editedTimeline.sections.find((s) => s.id === row.sectionId)?.tasks
													.length ?? 0)
											: 0;
									...
									const statusIcon = {
										todo: (
											<Circle size={13} className="text-stone-300 shrink-0" strokeWidth={1.5} />
										),
										"in-progress": (
											<Loader
												size={13}
												className="text-blue-400 shrink-0 animate-spin"
										...
										cancelled: (
											<Circle
												size={13}
												className="text-stone-300 shrink-0 line-through"
												strokeWidth={1.5}
											/>
										),
							...
							<div className="shrink-0" style={{ width: chartWidth }}>
								...
								>
									...
									{(() => {
										...
										const zoneSource =
											phasePositions.length > 0 ? phasePositions : milestonePositions;
										...
										for (let z = 0; z < edges.length - 1; z++) {
											...
											if (zw > 0)
												zones.push({
													...
													width: zw,
													index: z,
													label: sortedZoneMarkers[z]?.label,
												});
										}
										...
										const phaseAltFill = darkMode
											? "rgba(56,189,248,0.02)"
											: "rgba(14,165,233,0.015)";
									...
									{rows.map((row, i) => {
										...
										const task = row.task;
										const startX =
											daysBetween(rangeStart, parseDate(task.planned_start)) * DAY_WIDTH;
										const width =
											daysBetween(parseDate(task.planned_start), parseDate(task.planned_end)) *
											DAY_WIDTH;
										...
										if (task.actual_start && task.actual_end) {
											...
											if (actualDiffers) {
												actualStartX =
													daysBetween(rangeStart, parseDate(task.actual_start)) * DAY_WIDTH;
												actualWidth =
									...
									{announcementPositions.map((a, ai) => {
										...
										return (
											<g
												...
												onClick={(e) => {
													e.stopPropagation();
													setSelectedAnnouncement(isSelected ? null : a);
												}}
											>
												<line
													x1={a.x}
			...
			{showAddModal && (
				<AddItemModal
					...
					onClose={() => {
						...
						setAddModalDefaultSection(undefined);
					}}
					onCreated={() => onRefresh?.()}
					dark={darkMode}
